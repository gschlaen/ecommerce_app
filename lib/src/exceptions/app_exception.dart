...
extension AppExceptionDetails on AppException {
  AppExceptionData get details {
    return when(
      // Auth
      emailAlreadyInUse: () => AppExceptionData('email-already-in-use', 'Email already in use'.hardcoded),
      weakPassword: () => AppExceptionData('weak-password', 'Password is too weak'.hardcoded),
      wrongPassword: () => AppExceptionData('wrong-password', 'Wrong password'.hardcoded),
      userNotFound: () => AppExceptionData('user-not-found', 'User not found'.hardcoded),
      // Cart
      cartSyncFailed: () =>
          AppExceptionData('cart-sync-failed', 'An error has occurred while updating the shopping cart'.hardcoded),
      // Checkout
      paymentFailureEmptyCart: () =>
          AppExceptionData('payment-failure-empty-cart', 'Can\'t place an order if the cart is empty'.hardcoded),
      // Orders
      parseOrderFailure: (status) => AppExceptionData('parse-order-failure', 'Could not parse order status: $status'.hardcoded),
    );
  }
}
