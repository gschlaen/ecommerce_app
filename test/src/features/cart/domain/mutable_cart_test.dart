...
void main() {
  group('set item', () {
    ...
    test('cart with same item - override quantity', () {
      final cart = const Cart().addItem(const Item(productId: '1', quantity: 1)).setItem(const Item(productId: '1', quantity: 2));
      expect(cart.items, {'1': 2});
    });
    test('cart with different item - set item with quantity', () {
      final cart = const Cart().addItem(const Item(productId: '2', quantity: 1)).setItem(const Item(productId: '1', quantity: 2));
      expect(cart.items, {
        '2': 1,
        '1': 2,
      });
  ...
  group('add item', () {
    ...
    test('empty cart - add two items', () {
      final cart = const Cart().addItem(const Item(productId: '1', quantity: 1)).addItem(const Item(productId: '2', quantity: 1));
      expect(cart.items, {
        '1': 1,
        '2': 1,
      });
    ...
    test('empty cart - add same item twice', () {
      final cart = const Cart().addItem(const Item(productId: '1', quantity: 1)).addItem(const Item(productId: '1', quantity: 1));
      expect(cart.items, {'1': 2});
    });
  });
